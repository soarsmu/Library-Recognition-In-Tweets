...
with open(os.path.join(os.path.dirname(__file__), './apidoc/pd-np-mpl-realamb.txt'), 'r') as neg:
    for line in neg:
        if line != '\n':
            line = line.strip()
            api_neg.append(line)
...
with open(os.path.join(os.path.dirname(__file__), './apidoc/pd-np-mpl-remove.txt'), 'r') as pos:
    for line in pos:
        if line != '\n':
            line = line.strip()
            line = line.lower()
